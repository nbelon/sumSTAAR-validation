...
phen <- fread('/home/lima/Zor/SUM_STATISTICS/sumSTAAR/CAD/I25cov.nonrel.ord.csv', h = T, data.table = F)
...
Geno <- read.plink('/home/lima/Zor/SUM_STATISTICS/sumSTAAR/CAD/white_nonrel_chr01',select.snps = c(start:end))
...
##Last dose
start = nvar*ndose+1
Geno <- read.plink('/home/lima/Zor/SUM_STATISTICS/sumSTAAR/CAD/white_nonrel_chr01',select.snps = c(start:M))
gt <- as(Geno$genotype,'numeric')
pval <- Indiv_Score_Test_Region(genotype=gt,obj_nullmodel,rv_num_cutoff = 5,rare_maf_cutoff = 1)
ind.pval <- rbind (ind.pval,pval)
...
print('read map file')
map <- fread('/home/lima/Zor/SUM_STATISTICS/sumSTAAR/CAD/white_nonrel_chr01.bim', data = F, h = F)
map <- map[,-c(3)]
...
gwas <- fast[, c('CHR', 'SNP', 'POS', 'A1', 'AF1', 'BETA', 'SE', 'pz')]
colnames(gwas)[1:8] <- c('CHR','ID','POS','EA','EAF','BETA','SE','P')
...
list <- fread('~/Zor/SUM_STATISTICS/sumSTAAR/staar/chr01/snplist4gb_mac5.maf_0.01.txt',h = T, data.table = F)
...
for (i in 1:dim(gens)[1]) {
	...
	if (length(snps)> 1){
		...
		Geno <- read.plink('/home/lima/Zor/SUM_STATISTICS/sumSTAAR/CAD/white_nonrel_chr01',select.snps = snps)
		gt <- as(Geno$genotype,'numeric')
...
draw.qq <- function(pval0, pval1, main) {

	chi0 <- qchisq((1 - pval0), 1)
	lam0 <- median(chi0)/qchisq(0.5, 1)
	chi1 <- qchisq((1 - pval1), 1)
	lam1 <- median(chi1)/qchisq(0.5, 1)
	...
	plot(x, pval0, type = "n", ylab = ylb, xlab = xlb, ylim = c(0, ceiling(max(c(pval0, pval1)))), main = main)
	# upper and lower have already been subset
	polygon(-log10(c(b, rev(b))), -log10(c(upper, rev(lower))), density=NA, col="gray80")
	points(x, pval0, pch = 19, col = 'black')
	points(x, pval1, pch = 19, col = 'darkred')
	abline(0, 1)
	legend('topleft', legend = c(paste0('Lambda STAAR = ', round(lam0, 3)), paste0('Lambda sumSTAAR = ', round(lam1, 3))), pch = 19, bty = "n", col = c('black', 'darkred'), cex = .9)
...
pval0 <- sort(as.numeric(res0[, c('STAAR.B11', 'STAAR.B125')])) # sort() removes NAs
pval1 <- sort(as.numeric(res0[, c('sumSTAAR.B11', 'sumSTAAR.B125')]))
draw.qq(pval0, pval1, 'Burden tests')

pval0 <- sort(as.numeric(res0[, c('STAAR.S11', 'STAAR.S125')]))
pval1 <- sort(as.numeric(res0[, c('sumSTAAR.S11', 'sumSTAAR.S125')]))
draw.qq(pval0, pval1, 'SKAT')

pval0 <- sort(as.numeric(res0[, c('STAAR.A11', 'STAAR.A125')]))
pval1 <- sort(as.numeric(res0[, c('sumSTAAR.A11', 'sumSTAAR.A125')]))
draw.qq(pval0, pval1, 'ACAT')

pval0 <- sort(as.numeric(res0[, 'STAAR.ACAT_O']))
pval1 <- sort(as.numeric(res0[, 'sumSTAAR.ACAT_O']))
draw.qq(pval0, pval1, 'Combined tests')
